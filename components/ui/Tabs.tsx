...
export const Tabs: React.FC<TabsProps> = ({ items, activeKey, onChange, variant = 'underline' }) => {
  if (variant === 'segmented') {
    return (
      <div className="relative z-20 grid grid-cols-2 gap-4 p-1 bg-slate-900 rounded-lg border border-slate-800">
        {items.map(it => (
          <button
            key={it.key}
            data-key={it.key}
            onClick={() => { console.log('Tabs click', it.key); onChange(it.key); }}
            className={`text-xs py-1.5 px-2 rounded-md transition-all flex items-center justify-center gap-2 cursor-pointer z-10 ${activeKey === it.key ? 'bg-slate-700 text-white shadow' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-700/50'}`}
          >
            {it.icon}
            {it.label}
          </button>
